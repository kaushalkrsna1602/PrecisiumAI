...
export async function hasActivePlan(email:string) {
  ...
  const query = await sql` SELECT price_id , status
    FROM users
...
export async function hasReachedUploadLimit(userId: string): Promise<{ hasReachedLimit: boolean; uploadLimit: number }> {
