...
import { pricingPlans, PriceType, containerVariants, itemVariants } from "@/utils/constants";
...
const PricingCard = ({name, price, description, items, id, paymentLink, priceId}: PriceType) => {
    return (
        <MotionDiv variants={listVariants} whileHover={{scale : 1.02}} className="relative w-full max-w-lg hover:scale-105 hover:transition-all duration-300">
            <div className={cn('relative flex flex-col h-full gap-4 lg:gap-8 z-10 p-8 border-[1px] border-gray-500/20 rounded-2xl' , id === 'pro' && 'border-rose-500 border-2')}>
            ...
            <MotionDiv variants={listVariants} className="sapce-y-2.5 leading-relaxed text-base felx-1">
                {items.map((item , idx) => (
            ...
            <MotionDiv variants={listVariants} className="space-y-2 flex justify-center w-full">
                ...
                href={paymentLink}>Buy Now <ArrowRight size={18} />
                </Link>
            </MotionDiv>
            </div>
        </MotionDiv>
